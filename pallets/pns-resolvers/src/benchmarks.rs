...
use crate::resolvers::{Address, Call, Config, Content, Pallet, TextKind, Accounts};
...
use pns_types::{DomainHash, ddns::codec_type::RecordType};
...
benchmarks! {
    ...
    set_record {
        let l in 0..1024;
        let (owner,node) = get_cupnfish_node::<T>()?;
        let data = Content(polkadot_sdk::sp_std::vec![7;l as usize]);
    }: _(RawOrigin::Signed(owner), node, RecordType::A, data)
    ...
    remove_account {
        let (owner,node) = get_cupnfish_node::<T>()?;
        let address = Address::Id(owner.clone());
        Accounts::<T>::insert(node, &address, ());
    }: _(RawOrigin::Signed(owner), node, address)
}
...
fn account_to_source<T: polkadot_sdk::frame_system::Config>(
    account: T::AccountId,
...
where
    T: polkadot_sdk::frame_system::Config,
    C: polkadot_sdk::frame_support::traits::Currency<T::AccountId>,
{
