...
fn get_name(len: usize) -> polkadot_sdk::sp_std::vec::Vec<u8> {
...
pub fn account_to_source<T: polkadot_sdk::frame_system::Config>(
    account: T::AccountId,
...
pub fn poor_account<T: polkadot_sdk::frame_system::Config>(idx: u32) -> T::AccountId {
...
mod registry {
    ...
    use polkadot_sdk::frame_benchmarking::Zero;
...
mod registrar {
    ...
    where
        T: polkadot_sdk::frame_system::Config + polkadot_sdk::pallet_balances::Config,
    {
        let caller: T::AccountId = account("caller", idx, SEED);
        polkadot_sdk::pallet_balances::Pallet::<T>::set_balance(
            RawOrigin::Root.into(),
    ...
    benchmarks! {
        where_clause {
            where
            T: crate::origin::Config + polkadot_sdk::pallet_balances::Config,
        }
        add_reserved {
        ...
        }:_(RawOrigin::Signed(rich_account))
...
mod redeem_code {
    ...
    benchmarks! {
        ...
        name_redeem_min {
            let name = polkadot_sdk::sp_std::vec![104, 120, 120];
            let duration = T::Moment::from(31536000_u32);
            let nouce = 5;
            let signature = T::Signature::decode(&mut &polkadot_sdk::sp_std::vec![0, 229, 199, 81, 157, 241, 4, 157, 210, 38, 135, 222, 235, 38, 34, 192, 103, 30, 22, 80, 103, 169, 1, 150, 27, 177, 180, 162, 166, 18, 199, 178, 147, 115, 83, 174, 148, 221, 52, 101, 44, 22, 46, 84, 126, 48, 154, 45, 106, 125, 139, 217, 17, 59, 243, 210, 11, 77, 46, 200, 216, 98, 238, 110, 8][..]).unwrap();
            let official = T::AccountId::decode(&mut &polkadot_sdk::sp_std::vec![13, 213, 60, 222, 83, 155, 9, 162, 203, 198, 116, 100, 154, 230, 209, 84, 224, 76, 72, 25, 6, 39, 161, 214, 157, 32, 78, 221, 137, 199, 207, 162][..]).unwrap();
        ...
        name_redeem_any_min {
            let name = polkadot_sdk::sp_std::vec![99, 117, 112, 110, 102, 105, 115, 104, 120, 120];
            let duration = T::Moment::from(31536000_u32);
            let nouce = 5;
            let signature = T::Signature::decode(&mut &polkadot_sdk::sp_std::vec![0, 182, 166, 0, 120, 22, 9, 41, 218, 6, 241, 55, 33, 5, 184, 6, 196, 87, 25, 50, 80, 73, 5, 245, 146, 120, 185, 202, 248, 52, 213, 24, 175, 10, 58, 41, 114, 237, 190, 72, 138, 70, 221, 151, 104, 249, 219, 191, 135, 243, 221, 29, 240, 231, 197, 177, 246, 248, 213, 114, 169, 60, 99, 167, 2][..]).unwrap();
            let official = T::AccountId::decode(&mut &polkadot_sdk::sp_std::vec![13, 213, 60, 222, 83, 155, 9, 162, 203, 198, 116, 100, 154, 230, 209, 84, 224, 76, 72, 25, 6, 39, 161, 214, 157, 32, 78, 221, 137, 199, 207, 162][..]).unwrap();
